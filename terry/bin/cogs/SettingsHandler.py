...
class SettingsHandler(Cog):
    ...
    async def logging_system(self, ctx, mode: Optional[str]):
        ...
        if mode == 'on' or mode == 'off':
            ...
            emb = Embed(color=0x6b32a8,
                        ...
                        description='Отправляет информацию о новых пользователях, вход/выход, баны и тд.')
            emb.set_thumbnail(url='https://img.icons8.com/dusk/64/000000/settings.png')
    ...
    @send_settings.group(name='channel', aliases=['channels', 'ch', 'chan'])
    ...
    async def role_system(self, ctx):
        if not ctx.invoked_subcommand:
            ...
            emb.set_thumbnail(url='https://img.icons8.com/dusk/64/000000/settings.png')
            emb.add_field(name='Для изменения роли гостя', value=f'```{PREFIX}settings role guest```', inline=False)
    ...
    async def guest(self, ctx, role: Optional[Role]):
        role_db = await get_role(ctx.guild.id, 'guest_role')

        role_db_id = role_db.id or None
