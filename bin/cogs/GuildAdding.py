...
class GuildAdding(Cog):
    ...
    async def add_guild_to_db(self, ctx):
        ...
        if not results:
            ...
            logger.info(f'Added a new guild. ID: {ctx.guild.id}, Name: ' +
                        '{ctx.guild.name}')
    ...
    async def on_guild_join(self, guild):
        ...
        if not results:
            ...
            logger.info(f'Added a new guild. ID: {guild.id}, Name: ' +
                        '{guild.name}')

        elif results:
            logger.info('Guild is already exist in DB. ID: {guild.id}, Name:' +
                        '{guild.name}')
